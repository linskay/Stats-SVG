...
const image_base64 = fs.readFileSync(
  path.join(__dirname, "../asset/image.gif"),
  "base64",
);
...
async function calculateRankRing(config, stats, svg_width, svg_height) {
  ...
  const rank_ring_center_y = Math.round(
    svg_height / 2 - rank_ring_radius * 1.2,
  );
...
async function calculateLanguageRing(config, svg_width, svg_height) {
  ...
  const language_ring_center_y = Math.round(
    svg_height / 2 + language_ring_radius * 2,
  );
  const language_circumference = Math.round(2 * Math.PI * language_ring_radius);
  const first_column_x_offset = Math.round(
    language_ring_center_x + language_ring_radius * 1.8,
  );
  const second_column_x_offset = Math.round(
    first_column_x_offset + (1080 - first_column_x_offset) / 2,
  );
  return {
...
async function calculateImagePosition(
  dimensions,
  language_ring_radius,
  language_ring_thickness,
  language_ring_center_x,
  ...
  const target_height = Math.round(
    language_ring_radius * 2 - language_ring_thickness,
  );
  const image_y = Math.round(language_ring_center_y - target_height / 2);
  const image_x = Math.round(
    language_ring_center_x - ((target_height / image_height) * image_width) / 2,
  );
  return { target_height, image_y, image_x };
}

async function renderLanguageRing(
  languagePercentages,
  languageRingConfig,
  elementsConfig,
) {
  ...
  return languagePercentages
    ...
    `;
      ...
      const column_x_offset = isFirstColumn
        ? languageRingConfig.first_column_x_offset
        : languageRingConfig.second_column_x_offset;
      const column_index = isFirstColumn ? index : index - 10;
      const text_y_position = Math.round(
        languageRingConfig.language_ring_center_y -
          languageRingConfig.language_ring_radius -
          8 +
          column_index * 19,
      );
      ...
      <g transform="translate(${column_x_offset}, ${text_y_position})" class="animate" style="animation-delay: ${index * 0.15}s;">
        ...
        <text x="20" y="8" class="language-legend" dominant-baseline="central">
          <tspan fill="${elementsConfig.text_label_color}">${safeLanguage}</tspan>
...
async function renderContributionChart(
  ...
  // Sort the dates
  const sortedDates = Object.keys(contributionDistribution)
    .sort()
    .slice(-config.contribution_distribution.days_to_show);
  const data = sortedDates.map((date, i, arr) => {
    const total = contributionDistribution[date].total;
    const prevTotal =
      i > 0 ? contributionDistribution[arr[i - 1]].total : total;
  ...
  const chartX =
    rankRingConfig.rank_ring_center_x + rankRingConfig.rank_ring_radius * 1.8;
  const chartWidth = 1080 - chartX;
  const chartY =
    rankRingConfig.rank_ring_center_y - 1.2 * rankRingConfig.rank_ring_radius;
  ...
  const totalBarDisplayTime =
    (data.length - 1) *
    config.contribution_distribution.bar_display_time_interval;
    ...
    ${data
      .map((d, i) => {
        ...
        if (sameLevel) {
          ...
          color =
            d.close > d.open
              ? config.contribution_distribution.bullish_color
              : config.contribution_distribution.bearish_color;
        }
...
async function renderStats(stats) {
  const [
    ...
    calculateImagePosition(
      dimensions,
      config.language.ringRadius,
      config.language.ringThickness,
      Math.round(config.svg.width / 2),
      Math.round(config.svg.height / 2 + config.language.ringRadius * 2),
    ),
  ]);
